angular.module('eventApp')
    .controller('inoutHistory04', function ($scope,executeResults ,$http, $route, $rootScope, $location ,$routeParams) {
        $scope.total_count = [];
        $scope.total_cost = [];
        ...
        executeResults.day_move_company_out_list().then(function (data) {
            $scope.day_move_list = data;


            // 금일 물품별 sub list day_move_company_out_sub_list
            executeResults.day_move_company_out_sub_list().then(function (data) {
        ...
        executeResults.day_move_company_out_list_return().then(function (data) {
            $scope.day_move_list_return = data;

            // 금일 물품별 sub list day_move_company_sub_list_return
            executeResults.day_move_company_out_sub_list_return().then(function (data) {
                $scope.day_move_sub_list_return = data;
                if ($scope.day_move_sub_list_return[0].name !== "error") {

                    for (var i = 0; i < $scope.day_move_sub_list_return.length; i++) {
                        $scope.day_move_sub_list_return[i].total_name = $scope.day_move_sub_list_return[i].product_name + " " + $scope.day_move_sub_list_return[i].product_option;
                    };
                    ...
                    for (var i = 0; i < $scope.day_move_list_return.length; i++) {
                        ...
                        for (var j = 0; j < $scope.day_move_sub_list_return.length; j++) {
                            if($scope.day_move_list_return[i].company_id == $scope.day_move_sub_list_return[j].company_id){
                        ...
                        $scope.total_return_cost[i] = $.number($scope.total_return_cost[i]);
                    }
